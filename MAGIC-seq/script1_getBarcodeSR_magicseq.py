...
parser.add_argument("--bcz", default="", help="input barcodeZ whitelist")
parser.add_argument("-m", default=2, help="how many barcodes?")
...
def process(fq1, fq2, out1, out2, lookup_x, lookup_y,
            sx, ex, sy, ey, su, eu, min_qual=10, has_qual=False, lookup_z=None, sz=None, ez=None,
        ...
        with smart_open(fq1) as f1, smart_open(fq2) as f2, \
            ...
            for rec1, rec2 in zip(grouper(f1, 4), grouper(f2, 4)):
                ...
                if has_z:
                    if z_corrected:
                        stats["corrected_z"] += 1
                    if x_corrected or y_corrected or z_corrected:
                        stats["corrected_any"] += 1  
                    if x_corrected and y_corrected and z_corrected: 
                        stats["corrected_both_xyz"] += 1  
                else:
...
def main():
    ...
    t0 = time.time()
    if mode == 2:
        wl_x_path, wl_y_path = args.bcx, args.bcy
        sx, ex = 0, 8
        sy, ey = 26, 34
        su, eu = 34, 46

    elif mode == 3:
        wl_x_path, wl_y_path, wl_z_path = args.bcx, args.bcy, args.bcz
    ...
    t_idx = time.time() - t0
    print(f'[{time.strftime("%H:%M:%S")}] Now we start to count +^-^+')
    ...
    if mode == 3:
        print(
            f"reads discarded in step1--mismatch_Z: {stats['mismatch_z']}, "
            f"ratio among total: {pct(stats['mismatch_z'], total):.4f}%"
        )    
    ...
    if mode == 3:
        print(
            f"barcode corrected Z: {stats['corrected_z']}, "
            f"ratio among total: {pct(stats['corrected_z'], total):.4f}%, "
            f"ratio among kept: {pct(stats['corrected_z'], kept):.4f}%"
        )
        ...
        print(
            f"barcode corrected both X and Y and Z: {stats['corrected_both_xyz']}, "
            f"ratio among total: {pct(stats['corrected_both_xyz'], total):.4f}%, "
            f"ratio among kept: {pct(stats['corrected_both_xyz'], kept):.4f}%"
        )
